...
def binary_train(X, y, loss="perceptron", w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    if loss == "perceptron":
        ...
        for i in range(max_iterations):
            g = X.dot(w) + b
            h = (g > 0)
            # Dericative is -1 at 0
            h[(g == 0) & (y == 0)] = 1
            A = y - h
            A = A.reshape(N, 1)
            w = w + (step_size / N) * np.sum(A * X, axis = 0)
            b = b + (step_size / N) * np.sum(A, axis = 0)
...
def multiclass_train(X, y, C,
    ...
    if gd_type == "sgd":
        ...
        for it in range(max_iterations):
            ...
            p = p / np.sum(p, axis = 0)
            d = np.zeros((C, N))
            d[y,np.arange(y.size)] = 1
            p = p - d
            w = w - (step_size / N) * (p.dot(X))
            b = b - (step_size / N) * (np.sum(p, axis = 1, keepdims=True))
...
def multiclass_predict(X, w, b):
    ...
    #############################################################
    f = w.dot(X.T) + b.reshape(w.shape[0], 1)
    preds = np.argmax(f, axis = 0)
